...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    let required_extensions: InstanceExtensions = vulkano_win::required_extensions();
    let instance: Arc<Instance> = Instance::new(None, Version::V1_1, &required_extensions, None)?;
    let event_loop = EventLoop::new();

    let surface: Arc<Surface<Window>> =
        WindowBuilder::new().build_vk_surface(&event_loop, instance.clone())?;
